...
export const HomeCareHighlight: React.FC<HomeCareHighlightProps> = ({ onOpenTriagingModal }) => {
  const benefits = [
    {
      icon: <Home className="w-6 h-6 text-navy-900" />,
  ...
  return (
    <section id="domiciliar" className="py-20 md:py-28 bg-gradient-to-b from-sky-50/40 via-white to-slate-50 text-slate-900 relative overflow-hidden border-y border-slate-200/80">
      
      {/* Background Radial Ambient Glows */}
      <div className="absolute top-0 right-0 w-[500px] h-[500px] bg-sky-200/30 rounded-full blur-3xl pointer-events-none" />
      <div className="absolute bottom-0 left-0 w-[500px] h-[500px] bg-emerald-200/30 rounded-full blur-3xl pointer-events-none" />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 items-center">
          ...
          <div className="lg:col-span-7 space-y-6">
            ...
            <p className="text-base sm:text-lg text-slate-600 leading-relaxed">
              Para um idoso com doença neurológica ou dificuldade de deglutição, a alimentação é um momento delicado. O <strong className="text-slate-900 font-semibold">Atendimento Domiciliar da Dra. Caren Stefany</strong> garante que a reabilitação aconteça sem atropelos, na rotina real da família em <strong className="text-navy-900">Itatiba, Bragança Paulista, Morungaba e região</strong>.
            </p>

            {/* Handwritten badge accent */}
            <div className="py-2">
              <span className="handwriting text-2xl text-navy-800 font-bold tracking-wide">
            ...
            <div className="pt-4 flex flex-col sm:flex-row items-stretch sm:items-center gap-4">
              <button
                ...
                className="inline-flex items-center justify-center gap-3 bg-emerald-600 hover:bg-emerald-700 text-white font-bold px-8 py-4 rounded-2xl shadow-xl hover:shadow-emerald-600/20 transition-all text-base text-center cursor-pointer active:scale-[0.99]"
              >
          ...
          {/* Right Column Benefits List (Clean White Cards) */}
          <div className="lg:col-span-5 space-y-4">
            {benefits.map((benefit, idx) => (
              <div 
                key={idx}
                className="bg-white p-5 rounded-2xl border border-slate-200/90 shadow-sm hover:shadow-md hover:border-navy-400 transition-all"
              >
                <div className="flex items-start gap-4">
                  ...
                  <div>
                    <h3 className="font-bold text-base text-slate-900 mb-1 font-display">
                      {benefit.title}
                    </h3>
